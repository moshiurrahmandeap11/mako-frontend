  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className='mb-4'>
          <h1 className="text-xl sm:text-2xl font-normal text-[#222325] tracking-tight">Product Catalog</h1>
          <p className="text-[#62646A] text-xs sm:text-sm mt-1">Manage catalog items available for Labto AI recommendations</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <span className="flex items-center gap-2">
              <FileSpreadsheet className="w-4 h-4 text-[#74767E]" strokeWidth={1.5} />
              <span>Bulk Excel / CSV Import</span>
            </span>
          </Button>

          <Button onClick={() => setShowAddModal(true)} variant="primary" className="!font-normal text-xs sm:text-sm">
            <span className="flex items-center gap-2">
      ...
      <div className="relative">
        <Search className="w-4 h-4 text-[#74767E] absolute left-3.5 top-1/2 -translate-y-1/2" strokeWidth={1.5} />
        <input
          type="text"
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="text-center py-12 text-[#74767E] text-xs font-normal">
                    <Package className="w-8 h-8 mx-auto mb-2 opacity-40 text-[#74767E]" strokeWidth={1.5} />
                ...
                products.map((p) => (
                  <tr key={p.id} className="hover:bg-[#F7F7F7] transition">
                    <td className="py-3.5 px-4 flex items-center gap-3">
                      {p.imageUrl ? (
                        <img src={p.imageUrl} alt={p.title} className="w-9 h-9 rounded-md object-cover bg-slate-100 border border-[#E4E5E7]" />
                      ) : (
                      ...
                      <div>
                        <p className="font-normal text-[#222325] text-sm line-clamp-1">{p.title}</p>
                        <a href={p.productUrl} target="_blank" rel="noreferrer" className="text-xs text-[#1DBF73] hover:underline line-clamp-1 font-normal">
                          {p.productUrl}
                        </a>
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4">
                      <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-xs font-normal border ${p.inStock ? 'bg-[#F0F2F5] text-[#62646A] border-[#E4E5E7]' : 'bg-rose-50 text-rose-600 border-rose-200'}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${p.inStock ? 'bg-[#1DBF73]' : 'bg-rose-500'}`} />
                        {p.inStock ? 'In Stock' : 'Out of Stock'}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 font-mono text-xs text-[#74767E]">{p.externalId}</td>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-[#E4E5E7] rounded-md p-6 w-full max-w-xl space-y-4">
            <h2 className="text-base font-medium text-[#222325]">Add New Product</h2>
            ...
            <form onSubmit={handleCreateProduct} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-normal text-[#404145] mb-1">SKU / External ID *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-normal text-[#404145] mb-1">Title *</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-normal text-[#404145] mb-1">Price ($) *</label>
                  <input
                    type="number"
                    step="0.01"
                    required
                ...
                <div>
                  <label className="block text-xs font-normal text-[#404145] mb-1">Category</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs font-normal text-[#404145] mb-1">Product Page Link (URL) *</label>
                <input
                  type="url"
                  required
              ...
              <div>
                <label className="block text-xs font-normal text-[#404145] mb-1">Image Thumbnail Link (URL)</label>
                <input
                  type="url"
              ...
              <div>
                <label className="block text-xs font-normal text-[#404145] mb-1">Description</label>
                <textarea
                  rows={3}
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-[#E4E5E7]">
                <Button type="button" onClick={() => setShowAddModal(false)} variant="outline" className="text-[#222325] border-[#E4E5E7] !font-normal text-xs">
                  Cancel
                </Button>
                <Button type="submit" disabled={submitting} variant="primary" className="!font-normal text-xs">
            ...
            <form onSubmit={handleExcelImport} className="space-y-4">
              ...
              >
                ...
                <label htmlFor="excelFileInput" className="cursor-pointer space-y-2 block">
                  <FileSpreadsheet className={`w-10 h-10 mx-auto transition ${isDraggingExcel ? 'text-[#1DBF73]' : 'text-[#74767E]'}`} strokeWidth={1.5} />
                  ...
                  <p className="text-[11px] text-[#74767E]">Supports .xlsx, .xls, .csv files up to 10MB</p>
                </label>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2 border-t border-[#E4E5E7]">
                ...
                <Button type="submit" disabled={importing || !importFile} variant="primary" className="!font-normal text-xs">
