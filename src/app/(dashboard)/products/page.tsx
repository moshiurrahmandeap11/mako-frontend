...
export default function ProductsPage() {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-1">Manage products available for AI recommendation & search</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white rounded-xl text-xs font-semibold transition shadow-lg shadow-indigo-500/20"
          >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-11 pr-4 py-3 bg-slate-900 border border-slate-800 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition"
        />
      </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="text-center py-12 text-slate-500">
                    ...
                    No products found in catalog. Add your first item or upload JSON feed!
                  </td>
                </tr>
              ) : (
                products.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-850/50 transition">
                    <td className="py-4 px-6 flex items-center gap-3">
                      {p.imageUrl ? (
                        <img src={p.imageUrl} alt={p.title} className="w-10 h-10 rounded-lg object-cover bg-slate-800 border border-slate-700" />
                      ) : (
                        <div className="w-10 h-10 rounded-lg bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-400 text-xs">
                          📦
                        </div>
                      )}
                      <div>
                        <p className="font-semibold text-white text-sm line-clamp-1">{p.title}</p>
                        <a href={p.productUrl} target="_blank" rel="noreferrer" className="text-xs text-indigo-400 hover:underline line-clamp-1">
                          {p.productUrl}
                        </a>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${p.inStock ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'}`}>
          ...
          <div className="p-4 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
            ...
            <div className="space-x-2">
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-slate-800 border border-slate-700 disabled:opacity-50 text-white font-medium"
              >
                Previous
              </button>
              <button
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-xl shadow-2xl space-y-4">
            ...
            <form onSubmit={handleCreateProduct} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">SKU / External ID *</label>
                  <input
                    type="text"
                    required
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Product Page Link (URL) *</label>
                <input
                  type="url"
                  required
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Image Thumbnail Link (URL)</label>
                <input
                  type="url"
                ...
                placeholder={`[
  {
    ...
    "category": "Apparel"
                ...
                className="w-full p-3 font-mono text-xs bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-indigo-500"
              />
